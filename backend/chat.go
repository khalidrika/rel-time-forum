...
func (m *Manager) ChatHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, clients := range m.Users {
		for _, c := range clients {
			if c.Id != client.Id { // skip notifying self
				if err := c.Conn.WriteJSON(newUser); err != nil {
					log.Println("Failed to broadcast new user:", err)
				}
			}
		}
	}
	...
	for {
		...
		if msg.Type == "typing" || msg.Type == "stop_typing" {
			clients, ok := m.Users[msg.To]
			if ok {
				for _, c := range clients {
					if err := c.Conn.WriteJSON(msg); err != nil {
						log.Println("Failed to send typing message:", err)
					}
				}
			}
			continue
